...
int simul_rb_find(simul_rb_root *T, long long int find_key){
	int ret=0;
	simul_rb_node *search_node = T->root_node->rb_left;
...
//			long long int key = rand() % WSS;	// key range is small
	...
	if(strcmp(argv[2], "synthetic")==0){
		...
		strcpy(rfn,"/home/lmj/nvrbtree/microbench-v/wss/rvalue_");
		strcat(rfn,argv[3]);
		fp = fopen(rfn, "r");
		for(i=0; i<WSS; i++)
		...
		for(i=0; i<WSS; i++){
			s_key=rvalue[i];
			retval = simul_rb_find(root, (long long int)s_key);
			ser_count+=1;
		}
		for(i=0; i<WSS; i++){
			s_key=rvalue[i];
	...
	sizeofmem = cnt_node * (sizeof(unsigned int) + (sizeof(long long)) + (sizeof(struct simul_rb_node *)*3));
