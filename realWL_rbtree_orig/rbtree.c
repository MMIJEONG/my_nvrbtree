...
//#define WSS	1000000
unsigned long long int in_node = 0;//number of node after  insert
unsigned long long int cnt_node = 0;//number of node after the end
int WSS = 0;
int WSS1 = 0;
...
void init_rb_root(simul_rb_root *root) {
	...
	root->nil_node->rb_end = SIMUL_RB_NIL;
...
*/
	...
	root->root_node->rb_end = SIMUL_RB_NIL;
...
int simul_rb_insert(simul_rb_root *T, long long inkey, long long inend) {
	...
	init_rb_node(T, new_node, inkey, inend);
...
int simul_rb_find(simul_rb_root *T, long long find_key){
	...
	if(count1==0){
		if(find_key>pre_node->rb_key && find_key<pre_node->rb_end)
			return ret; 
	}
	return ret;
}	
...
int main(int argc, char *argv[])
{
	...
	FILE *fp = NULL;
	long long in_key = -1, i = 0;
	long long s_key=0, end_adr=0;
	...
	if (argc < 2) {
		...
		printf(" File is not given. try again\n");
...
//			long long int key = rand() % WSS;	// key range is small
	...
	if(strcmp(argv[1],"sample_trace")==0){
		printf("--run sample_trace mode --\n");
		...
		while(!feof(fp)){
			...
			if(strcmp(com,"insert")==0){
				...
				s_key=ivalue[0];
				end_adr=ivalue[1];
				retval=simul_rb_insert(root, (long long)s_key, (long long)end_adr);
				in_count+=1;
				//gettimeofday(&inend,NULL);
				//in_time+=inend.tv_sec - instart.tv_sec + (inend.tv_usec - instart.tv_usec) / 1000000.0; 	
				//in_time+=inend.tv_usec - instart.tv_usec;
			}
			else if(strcmp(com,"delete")==0){
				//gettimeofday(&delstart,NULL);
				s_key=ivalue[0];
				retval=simul_rb_delete(root, (long long)s_key);
				del_count+=1;
				//gettimeofday(&delend,NULL);
				//del_time+=delend.tv_sec - delstart.tv_sec + (delend.tv_usec - delstart.tv_usec)/1000000.0;
				//del_time+=delend.tv_usec - delstart.tv_usec;
			}
			else if(strcmp(com,"find")==0){
			 	//gettimeofday(&serstart,NULL);
				s_key=ivalue[0];
				retval=simul_rb_find(root, (long long)s_key);
				ser_count+=1;
	...
	sizeofmem = cnt_node * (sizeof(unsigned int) + (sizeof(long long)*2) + (sizeof(struct simul_rb_node *)*3));
	...
	printf(" number of node after insert : %llu\n", in_node);
        printf(" number of node after the end  : %llu\n", cnt_node);
        printf(" size of tree memory : %.4lf %s\n", transmemsize, byteunit[bytelevel]);
	printf(" Insert : %d, Delete : %d, Find : %d\n",in_count,del_count,ser_count);
	//printf(" Insertion time: %6.2f us\n", in_time);
	//printf(" Insertiong time: %6.2f us\n", inend.tv_usec - instart.tv_usec);
	//printf(" Deletion time: %6.2f us\n", del_time);
	//printf(" Delfetiong time: %6.2f us\n", delend.tv_usec - delstart.tv_usec);
