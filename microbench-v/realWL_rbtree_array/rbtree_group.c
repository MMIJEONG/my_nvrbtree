...
unsigned long long int total_write = 0;
...
int main(int argc, char *argv[])
{
	...
	long long int s_key=0, end_adr=0;
	double in_time = 0.0;
	double del_time = 0.0;
	double ser_time = 0.0;
...
//	init_swap_node();
	...
	if(strcmp(argv[1],"sample_trace")==0){
		...
		while(!feof(fp)){
			...
			if(strcmp(com,"insert")==0){
				...
				s_key=ivalue[0];
				end_adr=ivalue[1];
				retval=simul_rb_insert(root, (long long int)s_key);
				in_count+=1;	
			...
			if(strcmp(com,"delete")==0){
				...
				s_key=ivalue[0];
				retval=simul_rb_delete(root, (long long int)s_key);
				del_count+=1;
			...
			if(strcmp(com,"find")==0){ 
				...
				s_key=ivalue[0];
				ser_key = simul_rb_search_key(root, (long long int)s_key);
				ser_count+=1;
	...
	sizeofmem = cnt_node * (sizeof(unsigned int) + sizeof(long long int) + (sizeof(struct simul_rb_node *)*3) + sizeof(unsigned int) + (sizeof(long long int)*(GROUP_SIZE)));//mem_size measure  try again
	...
	printf(" TOTAL WRITE : %llu\n",total_write + color_write_count);
	//printf(" total write count : %llu\n", delete_write_count + remain_count + color_write_count);
	//printf(" WRITE : %llu\n", insert_write_count + delete_write_count + color_write_count);
