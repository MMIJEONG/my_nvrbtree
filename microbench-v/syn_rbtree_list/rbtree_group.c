...
//	init_swap_node();
	...
	if(strcmp(argv[3], "synthetic")==0){
		...
		for(i=0; i<WSS; i++){
			s_key=rvalue[i];
			search_node = simul_rb_search_key(root, (long long int)s_key);
			ser_count+=1;
		}
		for(i=0; i<WSS; i++){
			s_key=rvalue[i];
	...
	printf(" total write count : %llu\n", delete_write_count + remain_count + color_write_count + point_count);
	printf(" WRITE : %llu\n", insert_write_count + delete_write_count + color_write_count + point_count);
