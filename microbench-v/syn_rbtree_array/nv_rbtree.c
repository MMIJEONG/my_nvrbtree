...
void rb_init_node(rb_root *T, rb_node *node, long long int key) {
	set_rb_red(node);
	node->primary_key = key / (GROUP_SIZE);
	node->keys = (long long int *) malloc (sizeof(long long int) * (GROUP_SIZE));
	memset(node->keys, 0, sizeof(long long int) * (GROUP_SIZE));
	node->keys[key % (GROUP_SIZE)] = key;
	node->cnt_key = 1;
...
void rb_init_root(rb_root *root) {
	...
	root->nil_node->keys = (long long int *) malloc (sizeof(long long int) * (GROUP_SIZE));
	memset(root->nil_node->keys, 0, sizeof(long long int) * (GROUP_SIZE));
	...
	root->root_node->keys = (long long int *) malloc (sizeof(long long int) * (GROUP_SIZE));
	memset(root->nil_node->keys, 0, sizeof(long long int) * (GROUP_SIZE));
...
int rb_insert(rb_root *T, long long int inkey) {
	...
	if (is_in_group == 1) {
		comp_node->keys[inkey % (GROUP_SIZE)] = inkey;
		comp_node->cnt_key++;
...
int rb_delete(rb_root *T, long long int delkey) {
	...
	memset((void *)del_node->keys[delkey % (GROUP_SIZE)], 0, sizeof(long long int));
	del_node->cnt_key--;
